...
WWPO uses a centralized **Hub-and-Spoke (Master-Agent)** topology. The system is divided into two primary nodes: the **Master Node** (composed of a Go backend and React frontend dashboard) and the **Client Agent** (a native Go Windows Service).
...
* **Native Windows Service:** Runs as a Go Windows Service with elevated privileges (`SYSTEM` / `Administrators`).
...
sequenceDiagram
    ...
    participant Agent as Go Agent (Windows)
...
5. **Credentials Storing:** The Master returns the `Agent_ID` and the Connection Secret in the HTTPS response. The Go Agent saves these in a local configuration file with permissions restricted to `SYSTEM` and `Administrators`.
...
* **Ping/Pong Protocol:**
  ...
  * Every 30 seconds, the Go Agent pushes a `Ping` text payload or a WebSocket Ping frame.
...
The Go Agent enforces five core capabilities, prioritized by implementation urgency:
...
The Go Agent manages USB blocking by modifying registry keys under the Local Machine hive. To block storage entirely, the agent disables the kernel driver service for USB mass storage:
...
#### Self-Healing Engine Audit:
The Go Agent scans the registry rule subkeys. If a defined rule GUID is deleted or modified locally, the agent recreates the keys and values.
...
#### Enforcement Mechanics:
The Go Agent manages Windows Advanced Firewall configurations using the command-line utility `netsh.exe` invoked via command execution (`exec.Command`):
...
2. **Rule Enforcement:** Deploys rules based on the JSON configuration:
   * For each defined rule, the agent first purges any pre-existing rule of the same name:
     ```cmd
     netsh advfirewall firewall delete rule name="WWPO_<RuleName>"
     ```
   * It then appends the new configuration:
     ```cmd
     netsh advfirewall firewall add rule name="WWPO_<RuleName>" dir=<Direction> action=<Action> protocol=<Protocol> localport=<Port> remoteip=<RemoteIP>
     ```
   * Unmanaged rules (rules created by local users not matching the prefix `WWPO_`) can be disabled or deleted depending on baseline requirements.
...
#### Enforcement Mechanics:
Because local account security policies are protected within the Windows Security Accounts Manager (SAM) and Local Security Authority (LSA) databases, registry edits are ineffective. The Go Agent enforces these settings via **Secedit.exe**:
...
* **Privilege Minimums:** The Go Agent executes as `SYSTEM` / `Administrators`. All local configurations must have access control lists (ACLs) restricting writing access to SYSTEM or Administrators.
...
* **Local Event Logging:** The Go Agent logs status events to local standard logs (and writes to Event Viewer / syslog where configured) for native integration with local SIEMs.
